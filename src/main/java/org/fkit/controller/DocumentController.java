...
public class DocumentController{
	...
	// 上传文件会自动绑定到MultipartFile中
	 @RequestMapping(value="management/upload")
	 ...
	 @RequestMapping(value="management/uploadPic",method=RequestMethod.POST)
	 public String uploadPic(HttpServletRequest request,Model model,
		...
		if(!file.isEmpty()){
		    ...
		    model.addAttribute("filename",fileName);
		    model.addAttribute("information",information);
		    documentService.addPicture(fileName, information);
			// 判断路径是否存在，如果不存在就创建一个
	        if (!filepath.getParentFile().exists()) { 
	        	filepath.getParentFile().mkdirs();
	        }
	 ...
	 public ResponseEntity<byte[]> downloadPic(HttpServletRequest request,
